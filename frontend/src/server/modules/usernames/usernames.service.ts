...
import { resolveUsernameOnChain, usernameOfOnChain, server } from "../../../lib/stellar";
...
function isFresh(updatedAt: Date | undefined): boolean {
  return Boolean(updatedAt && Date.now() - updatedAt.getTime() < USERNAME_CACHE_TTL_MS);
}
...
async function upsertFromChain(username: string, onChain: OlioAccount): Promise<ResolveOutput> {
...
export async function resolveUsername(username: string): Promise<ResolveOutput> {
  const usernames = await getUsernames();
...
export async function registerUsernameCache(username: string): Promise<ResolveOutput> {
